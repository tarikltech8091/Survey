...
class QuestionAnswerController extends Controller
{
    ...
    ##  Store
     *********************************************/
    public function Store(Request $request, $surveyer_id, $campaign_id, $question_id)
    {
        ...
        if($v->passes()){
            ...
            try{
                ...
                $success = \DB::transaction(function () use($request) {
                    ...
                    $question_id = $request->input('answer_question_id');
                    $question_id = $request->input('answer_question_id');
                    $participate_mobile=$request->input('participate_mobile');
                    ...
                    $select_question = \DB::table('question_tbl')->where('id',$question_id)->where('question_status','1')->orderby('id','desc')->first();
                    ...
                    // var_dump($participate_insertOrUpdate);
                    // var_dump($campaign_participate_insertOrUpdate);
                    // var_dump($question_answer_insertOrUpdate);
                ...
                // return redirect()->to('/question/answer/'.$surveyer_id.'/'.$campaign_id.'/'.$question_id.'/2')->with('message','Question Answer Successfully');
                return redirect()->to('/all/question/answer/'.$participate_mobile.'/'.$surveyer_id.'/'.$campaign_id.'/2')->with('message','Question Answer Successfully');
        ...
        if($v->passes()){
            ...
            try{
                ...
                $success = \DB::transaction(function () use($request) {
                    ...
                    $select_surveyer = \DB::table('surveyer_tbl')->where('id',$surveyer_id)->where('surveyer_status','1')->orderby('id','desc')->first();
                    $select_campaign = \DB::table('campaign_tbl')->where('id',$campaign_id)->where('campaign_status','1')->orderby('id','desc')->first();
                    $select_question = \DB::table('question_tbl')->where('question_position',$question_position)->where('question_status','1')->orderby('id','desc')->first();
                    $all_question = \DB::table('question_tbl')->where('question_campaign_id',$campaign_id)->where('question_status','1')->orderby('id','desc')->get();
                ...
                return redirect()->to('/all/question/answer/'.$participate_mobile.'/'.$surveyer_id.'/'.$campaign_id.'/3')->with('message','Question Answer Successfully');
