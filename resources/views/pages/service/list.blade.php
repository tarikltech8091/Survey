                    ...
                    <form method="get"  action="{{url('/service/list')}}">
                        <div class="col-md-2">
                            <div class="form-group has-feedback ">
                                ...
                                <select class="form-control search-select" name="service_status">
                                    <option {{(isset($_GET['service_status']) && ($_GET['service_status']==1)) ? 'selected' : ''}} value="1">Publish</option>
                                    <option {{(isset($_GET['service_status']) && ($_GET['service_status']==0)) ? 'selected' : ''}} value="0">Unpublish</option>
                                </select>
                            </div>
                        </div>
                                        ...
                                        @if (isset($all_content)&& count($all_content)>0)
                                            ...
                                            @foreach ($all_content as $key => $list)
                                                <tr>
                                                    ...
                                                    <td>{{ $list->service_start_date }}</td>
                                                    <td>{{ $list->service_end_date }}</td>
                                                    <td>{{ $list->service_published_date }}</td>
                                                    <td>{{ str_limit($list->service_zone, 15)  }}</td>
                                                    ...
                                                    <td style="width:14%">
                                                        <div class="btn-group">
                                                            <button type="button" class="btn btn-purple"><i class="fa fa-wrench"></i> Action</button><button data-toggle="dropdown" class="btn btn-purple dropdown-toggle"><span class="caret"></span></button><ul class="dropdown-menu" role="menu">
                                                                <li>
                                                                    <a href="{{url('/service/edit/id-'.$list->id)}}">
                                                                        <i class="fa fa-pencil"></i> Edit
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    @if($list->service_status == 1)
                                                                        <a class="status-change" data-service-publish-status="0" data-service-id="{{ $list->id}}" title="Click for unpublish">
                                                                            <i class="fa fa-lock"></i> Unpublish
                                                                        </a>
                                                                    @else
                                                                        <a class="status-change " title="Click for publish" data-service-publish-status="1" data-service-id="{{ $list->id}}">
                                                                ...
                                                                <li>
                                                                    <a class="service-delete" data-service-id="{{$list->id}}">
    ...
    <script>
        $(function () {
            ...
            $('.status-change').on('click', function (e) {
                e.preventDefault();
                var service_publish_status = $(this).data('service-publish-status');
                var id = $(this).data('service-id');
                if(service_publish_status == 0) {
                    bootbox.dialog({
                        ...
                        buttons: {
                            ...
                            success: {
                                ...
                                callback: function() {
                                    $.ajax({
                                        type: 'GET',
                                        url: site_url+'/service/status/change/'+id+'/'+service_publish_status
                    ...
                    buttons: {
                        ...
                        danger: {
                            ...
                            callback: function() {
                                $.ajax({
                                    type: 'GET',
                                    url: site_url+'/service/delete/id-'+id,
                                }).done(function(response){
                                    bootbox.alert(response,
                                        function(){
                                            location.reload(true);
